...
{
    m_camera = camera;
    AppControl::projection = glm::perspective(glm::radians(m_camera->m_Fov), (float)Width / (float)Height, zbuffer_near, zbuffer_far);
    AppControl::view = m_camera->GetViewMatrix();
    AppControl::vp = AppControl::projection * AppControl::view;
    ...
    // ----------------------- normal model
    // model_Aru = new Model("./model/Aru/Aru.obj");
    // model_miku = new Model("./model/YYB Hatsune Miku_10th/YYB Hatsune Miku_10th_v1.02.pmx");
    // model_anon = new Model("./model/Anon- For The Rest Of Our Lives, Beginning Now/Anon.pmx");
    // ResourceManager::SetShader("model", 20, 24).Bind();
    // ResourceManager::GetShader("model").SetUniform4m("mvp", AppControl::vp * mat4(1.0));
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, AppControl::scr_width, AppControl::scr_height, 0, GL_RGBA, GL_HALF_FLOAT, NULL);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, AppControl::scr_width, AppControl::scr_height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
    glBindTexture(GL_TEXTURE_2D, 0);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, AppControl::scr_width, AppControl::scr_height, 0, GL_RED, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D, 0);
...
void Scene::Render()
{
    ...
    ResourceManager::GetShader("solid1").SetUniform4m("mvp", AppControl::vp * model);
    ...
    // // ResourceManager::GetShader("model").SetUniform4m("mvp", AppControl::vp * model);
    ...
    // draw red quad
    ResourceManager::GetShader("solid").SetUniform4m("mvp", AppControl::vp * redModelMat);
    ...
    ResourceManager::GetShader("transparent1").SetUniform4m("mvp", AppControl::vp * model);
    ...
    // draw green quad
    ResourceManager::GetShader("transparent").SetUniform4m("mvp", AppControl::vp * greenModelMat);
    ResourceManager::GetShader("transparent").SetUniform4f("color", glm::vec4(0.0f, 1.0f, 0.0f, 0.5f));
    ResourceManager::GetVAO("frame").Bind();
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);

    // draw blue quad
    ResourceManager::GetShader("transparent").SetUniform4m("mvp", AppControl::vp * blueModelMat);
    ...
    // ResourceManager::GetShader("model").Bind();
    // ResourceManager::GetShader("model").SetUniform4m("mvp", AppControl::vp * model);
    // shaderModel.SetUniform3m("normalMatrix", glm::mat3(glm::transpose(glm::inverse(view * model))));
    // model_anon->Draw(ResourceManager::GetShader("model"));
